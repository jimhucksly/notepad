...
export default class JSONFormatter {
  ...
  */
   ...
   *
  */
  toggleOpen(level: number) {
    this.isOpen = !this.isOpen;

    if (this.element) {
      if (this.isOpen) {
        this.appendChildren(this.config.animateOpen, level);
      } else{
  ...
  openAtDepth(depth = 1) {
    ...
    if (this.element) {
      ...
      if (depth === 0) {
        ...
        this.appendChildren(this.config.animateOpen, depth);
  ...
  */
  render(level = 1): HTMLDivElement {

    // construct the root element and assign it to this.element
    this.element = createElement('div', 'row', undefined, level);
    ...
    if (this.isObject && this.isOpen) {
      this.appendChildren(false, level + 1);
    }

    // add event listener for toggling
    if (this.isObject && !this.useToJSON) {
      togglerLink.addEventListener('click', this.toggleOpen.bind(this, level + 1));
    }
  ...
  */
  appendChildren(animated: boolean = false, level: number) {
    ...
    if (animated) {
      ...
      const addAChild = ()=> {
        ...
        children.appendChild(formatter.render(level));
