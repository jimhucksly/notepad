...
export default class Yandex extends Vue {
  ...
  async yandexCodeApply() {
    try {
      ...
      const query = new YandexTokenQuery(Number(this.yandexDiskResponseCode), Number(this.currentUser.id));
      ...
      message =
        (e as { message: string }).message || (e as { response: { message: string } }).response?.message || message;
