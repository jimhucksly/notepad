...
export default class Index extends Vue {
  ...
  async created() {
    const getUserPath = (): Promise<string> =>
      new Promise(resolve => {
        this.$electron.ipcRenderer.on('user-path-response', (e: Electron.IpcRendererEvent, value: string) => {
          resolve(value);
        });
  ...
  mounted() {
    this.$electron.ipcRenderer.on('download-start', (e: Electron.IpcRendererEvent) => {
      this.$store.commit('setProcess', { name: 'dowloading file...' });
    });
    this.$electron.ipcRenderer.on('download-end', (e: Electron.IpcRendererEvent) => {
      setTimeout(() => {
