...
export default class CalendarComponent extends Vue {
  ...
  private renderFirstWeek(e: Date): number {
    ...
    for (let i = weekday; i <= 6; i++) {
      ...
      const isHoliday = i >= 5;
      const isToday =
        num === Number(this.baseDay) &&
        Number(currMon) === Number(this.baseMonth) &&
        Number(currYear) === Number(this.baseYear);
  ...
  private renderCalendar(e: Date): Date {
    ...
    let date: string;
    let n: number;
    const count = Number(this.op.daysCount[currMon]);
    const weeksCount = 5;
    const mon = this.op.monIndex[currMon];
    const nextMon = Number(mon) === 12 ? '01' : this.op.monIndex[currMon + 1];
    const yearOfNextMon = Number(mon) === 12 ? currYear + 1 : currYear;
    ...
    for (let j = 1; j <= weeksCount; j++) {
      ...
      for (let i = 0; i <= 6; i++) {
        ...
        const isDisabled = this.checkDisabled(date);
        const isToday =
          Number(number) === Number(this.baseDay) &&
          Number(currMon) === Number(this.baseMonth) &&
          Number(currYear) === Number(this.baseYear) &&
          !isNextMonth;
